...
class LoanController extends Controller {
    ...
    public function accessRules() {
        $accessRules = array(
            ...
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array('create', 'update','Approve','admin', 'delete'),
                'users' => array('@'),
            ),
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['Loan'])) {
            ...
            if ($model->save()) {
                ...
                $this->redirect(array('create', 'id' => $model->loanId));
            }
        }
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['Loan'])) {
            ...
            if ($model->save()) {
                Yii::app()->user->setFlash("success", "Record saved!");
                $this->redirect(array('admin', 'id' => $model->loanId));
            }
        }
    ...
    public function actionApprove($loanId, $code) {
        $loan = Loan::model()->findByPk($loanId);
        $loan->status = $code;
        $loan->save();
        if ($code == 1) {
            Yii::app()->user->setFlash('success', "Loan Approved, Notification to the customer has been sent.");
        } else {
            Yii::app()->user->setFlash('success', "Loan Rejected, Notification to the customer has been sent.");
        }
